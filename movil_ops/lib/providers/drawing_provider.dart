...
class DrawingProvider extends ChangeNotifier {
  ...
  bool _lastPointWasSnapped = false;
  bool get lastPointWasSnapped => _lastPointWasSnapped;
  ...
  void setDraftVertices(List<LatLng> points, {DrawingType type = DrawingType.hato, List<Hato>? existingHatos}) {
    _isDrawing = true;
    _drawingType = type;
    _parentHatoId = null;
    _draftVertices = List<LatLng>.from(points);
    _isEraserMode = false;
    _backupVertices = [];
    _lastPointWasSnapped = false;
    if (existingHatos != null) {
      _activeSnapPoints = GISService.collectAllSnapPoints(existingHatos, draftVertices: _draftVertices);
    }
    notifyListeners();
  }
  ...
  bool addVertex(LatLng rawPoint, AgroProvider agroProvider) {
    ...
    // 1. Obtener contexto de Hatos y Potreros
    ...
    // Actualizar puntos de snap magnéticos incluyendo el vértice inicial si ya hay trazado
    _activeSnapPoints = GISService.collectAllSnapPoints(
      existingHatos,
      draftVertices: _draftVertices,
    );

    // 2. Aplicar Snapping con prioridad (Vértices primero, luego Linderos)
    final LatLng snappedPoint = GISService.applyComprehensiveSnapping(
      rawPoint: rawPoint,
      vertexSnapPoints: _activeSnapPoints,
      isHato: isHato,
      existingHatos: existingHatos,
      parentHato: parentHato,
      siblingPotreros: siblingPotreros,
      vertexTolDegrees: GISService.snappingToleranceDegrees,
      edgeTolMeters: 40.0,
    );

    _lastPointWasSnapped = !GISService.arePointsEqual(snappedPoint, rawPoint);
