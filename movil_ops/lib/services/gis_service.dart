...
class GISService {
  ...
  /// Determina si dos segmentos se cruzan estrictamente en forma de X (cruce transversal)
  /// Retorna false si solo comparten un punto extremo o son colindantes/colineales sobre la misma línea
  static bool doSegmentsCrossStrictly(LatLng p1, LatLng q1, LatLng p2, LatLng q2) {
  ...
  static TopologyValidationResult validateNewVertex({
    ...
    if (currentVertices.isEmpty) {
      if (isHato) {
        // Al crear un HATO, el primer punto no debe caer dentro del interior de ningún Hato existente ni sus potreros
        for (final otherHato in existingHatos) {
          if (isPointInPolygonOrBoundary(newVertex, otherHato.vertices) &&
            ...
            return TopologyValidationResult.invalid(
              message: 'El punto inicial cae dentro del interior del Hato "${otherHato.nombre}".',
              conflictPoint: newVertex,
            );
          }
          for (final pot in otherHato.potreros) {
            if (isPointInPolygonOrBoundary(newVertex, pot.vertices) &&
                !isPointOnPolygonBoundary(newVertex, pot.vertices)) {
              return TopologyValidationResult.invalid(
                message: 'El punto inicial cae dentro del interior del Potrero "${pot.nombre}".',
                conflictPoint: newVertex,
              );
            }
          }
        ...
        // Al crear un POTRERO:
        // 1. Debe estar dentro de los límites del Hato padre
        if (parentHato != null) {
        ...
        // 2. No debe caer en el interior de ningún Potrero hermano existente
        final potrerosToCheck = siblingPotreros ?? parentHato?.potreros ?? [];
        for (final sib in potrerosToCheck) {
          if (isPointInPolygonOrBoundary(newVertex, sib.vertices) &&
              !isPointOnPolygonBoundary(newVertex, sib.vertices)) {
            return TopologyValidationResult.invalid(
              message: 'El vértice inicial cae en el interior del Potrero "${sib.nombre}".',
              conflictPoint: newVertex,
            );
          }
        }
    ...
    if (isHato) {
      for (final otherHato in existingHatos) {
        ...
        for (int i = 0; i < otherHato.vertices.length; i++) {
          ...
          if (doSegmentsCrossStrictly(lastVertex, newVertex, p1, p2)) {
            final conflict = getIntersectionPoint(lastVertex, newVertex, p1, p2) ?? newVertex;
            return TopologyValidationResult.invalid(
        ...
        for (final sample in samples) {
          if (isPointInPolygonOrBoundary(sample, otherHato.vertices) &&
              !isPointOnPolygonBoundary(sample, otherHato.vertices)) {
            return TopologyValidationResult.invalid(
              message: 'El tramo atraviesa el interior del Hato "${otherHato.nombre}".',
    ...
    if (!isHato && parentHato != null) {
      ...
      // No cruzar el perímetro del Hato transversalmente
      ...
      for (final sib in potrerosToCheck) {
        ...
        // También verificar que el tramo no pase por el interior del potrero vecino
        for (final sample in samples) {
          if (isPointInPolygonOrBoundary(sample, sib.vertices) &&
              !isPointOnPolygonBoundary(sample, sib.vertices)) {
            return TopologyValidationResult.invalid(
              message: 'El tramo atraviesa el interior del Potrero vecino "${sib.nombre}".',
              conflictPoint: sample,
              offendingSegment: newSegment,
            );
          }
      ...
      for (final otherHato in existingHatos) {
        ...
        for (int i = 0; i < vertices.length; i++) {
          ...
          for (int j = 0; j < otherHato.vertices.length; j++) {
            ...
            if (doSegmentsCrossStrictly(v1, v2, o1, o2)) {
    ...
    if (!isHato && parentHato != null) {
      ...
      for (int i = 0; i < vertices.length; i++) {
        ...
        for (int j = 0; j < parentHato.vertices.length; j++) {
          ...
          if (doSegmentsCrossStrictly(v1, v2, h1, h2)) {
        ...
        for (final sib in siblingPotreros) {
          ...
          for (int i = 0; i < vertices.length; i++) {
            ...
            for (int j = 0; j < sib.vertices.length; j++) {
              ...
              if (doSegmentsCrossStrictly(v1, v2, s1, s2)) {
  ...
  // ==========================================
  // 6. SISTEMA DE SNAPPING (IMANES A VÉRTICES Y LINDEROS)
  // ==========================================

  /// Proyecta ortogonalmente un punto P sobre el segmento AB y lo acota al intervalo [A, B]
  static LatLng projectPointToSegment(LatLng p, LatLng a, LatLng b) {
  ...
  static List<LatLng> collectAllSnapPoints(List<Hato> hatos, {String? excludeHatoId, String? excludePotreroId, List<LatLng>? draftVertices}) {
    ...
    // Incluir el primer vértice del polígono actual en dibujo para permitir cerrar fácilmente el bucle
    if (draftVertices != null && draftVertices.isNotEmpty) {
      points.add(draftVertices.first);
    }
  ...
  /// Aplica snapping al lindero o segmento más cercano si la distancia es menor a tolMeters
  static LatLng applyEdgeSnapping(LatLng target, List<List<LatLng>> polygons, [double tolMeters = 40.0]) {
    LatLng closest = target;
    ...
    for (final poly in polygons) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        final proj = projectPointToSegment(target, p1, p2);
        final dist = calculateDistanceM(target, proj);

        if (dist <= tolMeters && dist < minDistanceM) {
          minDistanceM = dist;
          closest = proj;
        }
      }
  ...
  /// Sistema de Snapping Unificado (Vértices + Linderos)
  /// 1. Prioridad 1: Vértices/esquinas existentes (Snap magnético a puntos)
  /// 2. Prioridad 2: Linderos (Edge snapping ortogonal a cercas del Hato padre o Potreros vecinos)
  static LatLng applyComprehensiveSnapping({
    required LatLng rawPoint,
    required List<LatLng> vertexSnapPoints,
    required bool isHato,
    required List<Hato> existingHatos,
    Hato? parentHato,
    List<Potrero>? siblingPotreros,
    double vertexTolDegrees = snappingToleranceDegrees,
    double edgeTolMeters = 40.0,
  }) {
    // 1. Probar Snap a Vértice existente (máxima prioridad)
    final vertexSnapped = applySnapping(rawPoint, vertexSnapPoints, vertexTolDegrees);
    if (!arePointsEqual(vertexSnapped, rawPoint)) {
      return vertexSnapped;
    }

    // 2. Probar Snap a Borde / Lindero de cercas existentes
    final List<List<LatLng>> polygonsToSnap = [];
    if (isHato) {
      for (final h in existingHatos) {
        if (h.vertices.length >= 2) {
          polygonsToSnap.add(h.vertices);
        }
      }
    } else {
      if (parentHato != null && parentHato.vertices.length >= 2) {
        polygonsToSnap.add(parentHato.vertices);
      }
      final potreros = siblingPotreros ?? parentHato?.potreros ?? [];
      for (final p in potreros) {
        if (p.vertices.length >= 2) {
          polygonsToSnap.add(p.vertices);
        }
      }
      for (final h in existingHatos) {
        if (h.vertices.length >= 2 && !polygonsToSnap.contains(h.vertices)) {
          polygonsToSnap.add(h.vertices);
        }
      }
    }
